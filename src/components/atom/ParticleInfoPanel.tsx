...
export function ParticleInfoPanel({
  ...
  return (
    <div className="glass-panel-subtle flex flex-col gap-5 rounded-2xl p-5">
      {/* Active model + block, so the panel always reflects what's on screen. */}
      <div className="flex flex-wrap items-center gap-2">
        <span className="inline-flex items-center gap-1.5 rounded-full border border-accent/40 bg-accent/10 px-2.5 py-0.5 text-[0.7rem] font-medium text-[#6cb4ff]">
      ...
      <div>
        <div className="flex items-center justify-between gap-2">
          ...
          {copy && (
            ...
            >
              Clear <Close className="h-3.5 w-3.5" />
            </button>
          )}
        </div>
      ...
      <div>
        <h4 className="text-[0.65rem] font-semibold uppercase tracking-[0.1em] text-muted-2">
          Electron configuration
        </h4>
      ...
      <div className="grid grid-cols-3 gap-2">
        <CountStat label="Protons" value={protons} dot="#ff6b84" />
        <CountStat label="Neutrons" value={neutrons} dot="#7fadd1" />
        <CountStat label="Electrons" value={electrons} dot="#64d2ff" />
      </div>
      ...
      <div>
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {shells.map((count, index) => (
            <span
              key={index}
              className="inline-flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/[0.05] px-2.5 py-1 text-xs"
            >
...
function FactCell({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2">
      <span className="block text-[0.65rem] uppercase tracking-[0.08em] text-muted-2">
        {label}
      </span>
